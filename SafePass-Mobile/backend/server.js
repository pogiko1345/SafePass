...
const sendStudentGuardianAttendanceEmail = async ({ user, action, timestamp, status }) => {
  const normalizedRole = normalizeUserRoleValue(user?.role);
  if (normalizedRole !== "student") {
    return { success: false, skipped: true, reason: "not_student" };
  }

  const guardianEmail = normalizeEmailValue(user?.guardianEmail || "");
  if (!guardianEmail) {
    return { success: false, skipped: true, reason: "guardian_email_not_configured" };
  }

  const studentName = getFullName(user) || user.email || "Student";
  const actionLabel = action === "check_out" ? "left campus" : "entered campus";
  const timeLabel = new Date(timestamp).toLocaleString();
  const subject = `SafePass campus update for ${studentName}`;
  const body = [
    `Hello ${user?.guardianName || "Guardian"},`,
    "",
    `${studentName} ${actionLabel} on ${timeLabel}.`,
    `Attendance status: ${status}.`,
    "",
    "This update was generated by SafePass after a student NFC attendance tap.",
  ].join("\n");
...
app.post("/api/device/location-tap", validateDeviceKey, async (req, res) => {
  try {
    ...
    const cardUser = await User.findOne({
      ...
      "_id email firstName lastName nfcCardId role status accessPermissions department position guardianName guardianEmail guardianPhone smsOptIn scheduleProfile",
    );
